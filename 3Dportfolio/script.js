...
function generateCarousel(items) {
    ...
    allItems.forEach(item => {
        ...
        if (item.type === 'image') {
            ...
            // Уведомляем прелоадер о новом изображении
            if (window.preloaderManager) {
                window.preloaderManager.addResource('portfolio-image', item.src);
                
                img.onload = () => window.preloaderManager.markResourceLoaded(item.src);
                img.onerror = () => window.preloaderManager.markResourceLoaded(item.src);
            }
...
carousel.addEventListener('click', (e) => {
    ...
    // Исправленный поиск: ищем по окончанию пути (чтобы не зависеть от лишних директорий)
    const portfolioItem = portfolioItems.find(p => src.endsWith(p.src));
...
document.addEventListener('DOMContentLoaded', () => {
    const sections = document.querySelectorAll('.section, #works, .hero-model');
